...
Section Tarski.
  ...
  Definition exploding_bot D (I : interp D) := forall rho s t, rho ⊨ (⊥ --> Pred s t).
  ...
  Lemma semantic_explosion D {I : interp D} rho phi :
    exploding_bot I -> rho ⊨ ⊥ -> rho ⊨ phi.
  Proof.
    induction phi in rho |-*; unfold "⊨"; firstorder.
  Qed.
  ...
  Proof.
    ...
    intros H % Hdn. apply (semantic_explosion _ Hb H).
  Qed.
  ...
  Section Soundness.
    ...
    Proof.
      ...
      - eapply sat_comp, sat_ext. 2: apply (IHprv Hclass Hexpl D I HC rho HA (eval rho t)).
        now intros []; asimpl.
      - apply semantic_explosion; firstorder.
    ...
    Proof.
      ...
      - now intros _ D I [].
      - now intros _ D I [_ Hb] rho P t Hf % Hb. 
    Qed.
  End Soundness.
...
Proof.
  intros [] D [] He rho P v. cbn. erewrite vec_ext. 2: intros; rewrite <- (drop_interp'_eval (B_I i_f0 i_P0 i_F0)); reflexivity.
  erewrite <- @vec_comp. 2: reflexivity. apply (He rho P (Vector.map (sig_lift_term' (P_ar:=pred_ar)) v)).
Qed.
